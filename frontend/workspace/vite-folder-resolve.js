 ...
 * absolute path. We delegate the actual filesystem walk to the hyscience
 * backend at :4096 — only hyscience needs Full Disk Access on macOS, and
 * the user usually has it granted to that binary.
 *
 * Routes:
 *   GET  /api/resolve-folder/probe           — does hyscience see ~/Desktop?
...
const HYSCIENCE_BASE = "http://localhost:4096"
const HEADER = "x-hyscience-directory"
...
function authHeaders(headers = {}) {
  const password = process.env.HYSCIENCE_SERVER_PASSWORD
  if (!password) return headers
  const username = process.env.HYSCIENCE_SERVER_USERNAME || "hyscience"
  return {
    ...headers,
...
async function hyscienceList(directory) {
  try {
    const url = `${HYSCIENCE_BASE}/file?directory=${encodeURIComponent(directory)}&path=.`
    const res = await fetch(url, { headers: authHeaders({ [HEADER]: directory }) })
    if (!res.ok) {
      const fallback = await nodeList(directory)
      if (fallback.ok) return fallback
      return { ok: false, error: `hyscience ${res.status}` }
    }
...
async function probe() {
  // hyscience lists the parent fine but TCC blocks ~/Desktop unless the
  // hyscience binary has Full Disk Access. We treat "0 entries on Desktop"
  // as the user's Desktop being unreachable.
  const desktop = `${HOME}/Desktop`
  const r = await hyscienceList(desktop)
  if (!r.ok) return { fda: false, reason: r.error }
  const fda = r.entries.length > 0
  return { fda, reason: fda ? undefined : "hyscience returned 0 entries for ~/Desktop (TCC blocking)" }
}
...
async function findByName(name, hint, fingerprint) {
  ...
  for (const root of SEARCH_ROOTS) {
    const rootList = await hyscienceList(root)
    ...
    while (queue.length > 0 && candidates.length < MAX_CANDIDATES) {
      ...
      for (const d of cur.list.entries) {
        ...
        if (d.name === name) {
          ...
          const inner = await hyscienceList(full)
          if (inner.ok) {
        ...
        const next = await hyscienceList(full)
...
function readTitle(req) {
  const raw = new URL(req.url || "/", "http://localhost").searchParams.get("title")
  const title = String(raw || "Open project folder")
    .replace(/\\/g, "\\\\")
    .replace(/"/g, '\\"')
    .trim()
  return title || "Open project folder"
}

async function openNativeDialog(req) {
  if (process.platform === "darwin") {
    const title = readTitle(req)
    const script = [`set picked to choose folder with prompt "${title}"`, "POSIX path of picked"]
    const out = await run(
      "osascript",
...
async function validatePath(input) {
  ...
  const listed = await hyscienceList(real)
  return {
    ok: true,
    absolute: real,
    readable: listed.ok,
...
async function handle(req, res) {
  ...
  if (url.endsWith("/dialog")) {
    try {
      const r = await openNativeDialog(req)
