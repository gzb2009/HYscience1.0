...
const hyscienceDir = path.join(repoDir, "backend", "cli")
...
const sandbox = await fs.mkdtemp(path.join(os.tmpdir(), "hyscience-e2e-"))

// Pin Basic-Auth creds so the in-process server + the Playwright-hosted
// frontend (via VITE_HYSCIENCE_SERVER_PASSWORD) agree. Without this, flag.ts
...
const e2eServerUsername = "hyscience"
const e2eServerPassword = "hyscience-e2e-local-password"

const serverEnv = {
  ...process.env,
  HYSCIENCE_DISABLE_SHARE: "true",
  HYSCIENCE_DISABLE_LSP_DOWNLOAD: "true",
  HYSCIENCE_DISABLE_DEFAULT_PLUGINS: "true",
  HYSCIENCE_EXPERIMENTAL_DISABLE_FILEWATCHER: "true",
  HYSCIENCE_TEST_HOME: path.join(sandbox, "home"),
  XDG_DATA_HOME: path.join(sandbox, "share"),
  XDG_CACHE_HOME: path.join(sandbox, "cache"),
  XDG_CONFIG_HOME: path.join(sandbox, "config"),
  XDG_STATE_HOME: path.join(sandbox, "state"),
  HYSCIENCE_E2E_PROJECT_DIR: repoDir,
  HYSCIENCE_E2E_SESSION_TITLE: "E2E Session",
  HYSCIENCE_E2E_MESSAGE: "Seeded for UI e2e",
  HYSCIENCE_E2E_MODEL: "hysci/gpt-5-nano",
  HYSCIENCE_CLIENT: "app",
  HYSCIENCE_SERVER_USERNAME: e2eServerUsername,
  HYSCIENCE_SERVER_PASSWORD: e2eServerPassword,
...
const runnerEnv = {
  ...
  VITE_HYSCIENCE_SERVER_PORT: String(serverPort),
