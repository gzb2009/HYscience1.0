...
// Basic-Auth creds for the in-process hyscience server. e2e-local.ts pins both
// HYSCIENCE_SERVER_* (server side) and VITE_HYSCIENCE_SERVER_* (frontend side) to
// the same value so the Playwright-hosted frontend can authenticate.
const serverUsername = process.env.VITE_HYSCIENCE_SERVER_USERNAME ?? "hyscience"
const serverPassword = process.env.VITE_HYSCIENCE_SERVER_PASSWORD ?? ""
...
export default defineConfig({
  ...
  webServer: {
    ...
    timeout: 120_000,
    env: {
      VITE_HYSCIENCE_SERVER_HOST: serverHost,
      VITE_HYSCIENCE_SERVER_PORT: serverPort,
      VITE_HYSCIENCE_SERVER_USERNAME: serverUsername,
      VITE_HYSCIENCE_SERVER_PASSWORD: serverPassword,
    },
  },
  use: {
    baseURL,
    ...
    // hyscience-fetch.ts wraps fetch() with the same header, but its
