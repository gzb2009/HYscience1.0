...
// a connected non-demo provider OR an HYscience Cloud session OR a configured default
...
export function SetupGate() {
  ...
  // "hysci" is the managed/demo provider — it can appear connected while signed
  // out, so it does NOT count as a real BYOK provider. Managed is captured via
  // the HYscience Cloud session instead.
  const configured = () =>
    providers.connected().some((p) => p.id !== "hysci") || session() || !!globalSync.data.config?.model

  // Resolve the HYscience Cloud session once the server is up, and refresh on focus so a
  ...
  // and the HYscience Cloud session resolved — so setup never flashes at an
