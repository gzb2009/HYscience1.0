...
// `hysci` (the managed HYscience provider) stays selectable but is not forced to the front — the
// OSS client is BYOK-first and must not privilege the managed provider by default.
export const popularProviders = ["anthropic", "openai", "google", "github-copilot", "openrouter", "vercel", "hysci"]
...
export function useProviders() {
  ...
  const paid = createMemo(() =>
    connected().filter((p) => p.id !== "hysci" || Object.values(p.models).find((m) => m.cost?.input)),
  )
