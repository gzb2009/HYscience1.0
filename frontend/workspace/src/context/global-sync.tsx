...
import { retry } from "@hysci/util/retry"
import { useGlobalSDK } from "./global-sdk"
// InitError used to live in pages/error.tsx (now deleted with the legacy
// hyscience shell). Inline the shape so the hyscience context layer keeps
...
function createGlobalSync() {
  ...
  const sdkCache = new Map<string, ReturnType<typeof createHYscienceClient>>()
  const sdkFor = (directory: string) => {
    const cached = sdkCache.get(directory)
    if (cached) return cached

    const sdk = createHYscienceClient({
  ...
  async function loadSessionStatus(directory: string) {
    ...
    const promise = sdkFor(directory)
      .session.status()
      .then((x) => {
        const [, setStore] = child(directory, { bootstrap: false })
        setStore("session_status", x.data ?? {})
      })
      .catch(() => {})
      .finally(() => statusLoads.delete(directory))
