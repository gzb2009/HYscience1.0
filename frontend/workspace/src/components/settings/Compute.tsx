...
// under ~/.hyscience/ (AES-256-GCM, machine-local key) and never returned to the
...
import { Icon } from "@hysci/ui/icon"
    ...
    <div class="flex flex-col h-full overflow-y-auto no-scrollbar">
      <div class="sticky top-0 z-10 bg-[linear-gradient(to_bottom,var(--surface-raised-stronger-non-alpha)_calc(100%_-_24px),transparent)]">
        <div class="flex flex-col gap-1 px-4 py-8 sm:p-8 max-w-[820px]">
          ...
          <p class="text-13-regular text-text-weak">
            Choose where agent runs execute, then connect GPU keys or SSH login nodes as needed.
          </p>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-8 px-4 pb-12 sm:px-8 max-w-[820px]">
        <Section
          ...
          subtitle="Injected into research/biology/ml/physics turns as a system reminder."
        >
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <For
              each={
                [
                  { id: "local" as const, label: "Local", hint: "bash · notebook · rkernel" },
                  { id: "ssh" as const, label: "SSH / Slurm", hint: "remote + sbatch" },
                  { id: "cloud" as const, label: "Cloud", hint: "Modal · TensorPool · …" },
                ] as const
              }
            >
              {(tier) => {
                const active = () => (info()?.execution ?? "local") === tier.id
                return (
                  <button
                    type="button"
                    disabled={busy()}
                    onClick={() =>
                      run(
                        () =>
                          call<ComputeInfo>("/execution", {
                            method: "PUT",
                            body: JSON.stringify({ execution: tier.id }),
                          }),
                    ...
                    classList={{
                      "border-text-strong bg-surface-base": active(),
                      "border-border-weak-base hover:border-border-strong-base": !active(),
                    }}
                  >
                    <div class="text-13-medium text-text-strong">{tier.label}</div>
                    <div class="text-11-regular text-text-weak mt-1">{tier.hint}</div>
                  </button>
                )
              }}
            </For>
      ...
      <Show when={props.connecting}>
        <div class="flex flex-col gap-2">
          ...
          <p class="text-11-regular text-text-weak/70">
            Encrypted at rest under ~/.hyscience/ · never returned to the browser.
          </p>
        </div>
      </Show>
