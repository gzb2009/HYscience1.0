...
import { createEffect, createResource, createRoot, type Accessor } from "solid-js"
...
const GLOBAL_STORAGE = "hyscience.global.dat"
const LOCAL_PREFIX = "hyscience."
const fallback = { disabled: false }

/** Gives module-scope store effects a root so Solid does not warn about disposal. */
export function moduleEffect(fn: () => void) {
  createRoot(() => createEffect(fn))
}
...
function workspaceStorage(dir: string) {
  ...
  return `hyscience.workspace.${head}.${sum}.dat`
}
