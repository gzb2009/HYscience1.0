...
async function highlightCodeBlocks(html: string): Promise<string> {
  ...
  const [highlighter, bundledLanguages] = await Promise.all([
    getSharedHighlighter({ themes: ["HYscience"], langs: [] }),
    loadLangs(),
  ])
    ...
    return marked.use(
      ...
      markedShiki({
        async highlight(code, lang) {
          const highlighter = await getSharedHighlighter({ themes: ["HYscience"], langs: [] })
          ...
          return highlighter.codeToHtml(code, { lang: lang || "text", theme: "HYscience", tabindex: false })
        },
      }),
    )
  })())
