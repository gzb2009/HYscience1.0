...
| HYcloud session | `~/.config/hyscience/config.json` | `thk_*` key created by `hyscience login`; revocable from the dashboard. |
| Synced service credentials | `~/.config/hyscience/credentials.json` | Present only when [connected to HYcloud](/hyscience/cloud); refresh with `hyscience sync`. |
...
- Nothing else, in BYOK mode. If you [connect to HYcloud](/hyscience/cloud), synced credentials and usage metering are held against your account.
