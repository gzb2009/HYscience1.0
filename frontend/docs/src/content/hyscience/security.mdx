...
HYscience is open source under Apache-2.0, so the whole security model below is auditable in the [repository](https://github.com/HYscience/hyscience). The agent runs locally with the same filesystem and shell access you have. Two principles anchor the design: keep credentials out of subprocesses that do not need them, and make the trust boundary explicit instead of pretending the agent is a sandbox.

## The agent is not a sandbox

The permission system prompts you before the agent runs a command or writes a file. That keeps you aware of what it is doing — it is not an isolation boundary. The agent can read, write, and execute anywhere your user can. For real isolation, run HYscience inside a container or a VM.
...
| HYscience Cloud session | `~/.config/hyscience/config.json` | `thk_*` key created by `hyscience login`; revocable from the dashboard. |
| Synced service credentials | `~/.config/hyscience/credentials.json` | Present only when [connected to HYscience Cloud](/hyscience/cloud); refresh with `hyscience sync`. |
| Native binary (curl install) | `~/.hyscience/bin/hyscience` | Added to PATH through your shell rc. |

Override the config parent directory with `XDG_CONFIG_HOME`. Run `hyscience debug paths` to print the resolved data, config, cache, and state directories on your machine.
...
- Nothing else, in BYOK mode. If you [connect to Atlas](/hyscience/cloud), synced credentials and usage metering are held against your account.
...
Report security issues through the GitHub Security Advisory ["Report a Vulnerability"](https://github.com/HYscience/hyscience/security/advisories/new) form — not on public issue trackers. If you do not hear back within six business days, email security@hyscience.ai.
