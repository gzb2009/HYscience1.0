...
const SECTIONS: Section[] = [
  { key: "hyscience", label: "HYscience", short: "HYscience", tagline: "Open-source AI workbench", lead: false },
]
...
const ICONS: Record<string, ReactNode> = {
  ...
  cloud: <GitBranch size={17} strokeWidth={1.8} />,
...
const SECTION_FALLBACK_ICON: Record<SectionKey, ReactNode> = {
  hyscience: <FlaskConical size={17} strokeWidth={1.8} />,
}
...
const SECTION_DOC_PAGES: Record<SectionKey, Record<string, DocsPage>> = {
  hyscience: buildSectionPages("hyscience"),
}

const SECTION_CONFIGS: Record<SectionKey, DocsConfig> = {
  hyscience: RAW_CONFIGS["./content/hyscience/docs.json"],
}
...
// Retired first URL segments that redirect into the hyscience scheme.
const SECTION_ALIASES: Record<string, SectionKey> = {
  "agent-cli": "hyscience",
}

// Old CLI page names that moved during the HYscience rebuild.
...
const LEGACY_REDIRECTS: Record<string, { section: SectionKey; path: string }> = {
  "cli:index": { section: "hyscience", path: "index" },
  "cli:installation": { section: "hyscience", path: "quickstart" },
  "cli:quickstart": { section: "hyscience", path: "quickstart" },
  "cli:first-session": { section: "hyscience", path: "sessions" },
  "cli:sessions": { section: "hyscience", path: "sessions" },
  "cli:models": { section: "hyscience", path: "models" },
  "cli:codex": { section: "hyscience", path: "models" },
  "cli:sub-agents": { section: "hyscience", path: "agents" },
  "cli:skills": { section: "hyscience", path: "skills" },
  "cli:cli-runtime": { section: "hyscience", path: "commands" },
  "cli:connect": { section: "hyscience", path: "cloud" },
  "cli:credentials": { section: "hyscience", path: "cloud" },
  "cli:security": { section: "hyscience", path: "security" },
  "cli:feature-map": { section: "hyscience", path: "commands" },
  "cli:commands": { section: "hyscience", path: "commands" },
  "cli:web-ui": { section: "hyscience", path: "workspace" },
  "cli:server-mode": { section: "hyscience", path: "workspace" },
}

type Route = { section: SectionKey; path: string }

function defaultRoute(): Route {
  return { section: "hyscience", path: "index" }
}
...
function routeFromHash(): Route {
  ...
  // Retired section names redirect into the hyscience scheme.
...
let CURRENT_SECTION: SectionKey = "hyscience"
...
const HYSCIENCE_REPO = "HYscience/hyscience"
...
function GitHubStars() {
  ...
  useEffect(() => {
    let cancelled = false
    const cacheKey = `docs-gh-stars:${HYSCIENCE_REPO}`
    try {
    ...
    fetch(`https://api.github.com/repos/${HYSCIENCE_REPO}`)
  ...
  return (
    <div className="docs-ghstars">
      <a
        ...
        href={`https://github.com/${HYSCIENCE_REPO}`}
        target="_blank"
        rel="noreferrer"
      >
      ...
      <a href={`https://github.com/${HYSCIENCE_REPO}/blob/main/LICENSE`} target="_blank" rel="noreferrer">
        Apache-2.0
      </a>
      <a href="https://www.npmjs.com/package/@hysci/hyscience" target="_blank" rel="noreferrer">
        npm · @hysci/hyscience
      </a>
    </div>
  )
  ...
  return (
    <div className="docs-page">
      <header className="docs-topbar">
        <a href="https://hyscience.sh" className="docs-brand">
          <img src={`${import.meta.env.BASE_URL}favicon.svg`} alt="" />
          <span className="docs-brand-text">
            <small>HYscience</small>
            <strong>Docs</strong>
          </span>
        </a>
        ...
        <nav className="docs-actions" aria-label="documentation actions">
          ...
          <a
            ...
            href={config.navbar?.primary?.href ?? "https://github.com/HYscience/hyscience"}
          >
