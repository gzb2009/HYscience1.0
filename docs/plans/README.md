# HYscience × Atlas — improvement sprint (Phase 0: plans)

Branch: `sprint/hyscience-atlas-polish`. This directory holds one plan doc per workstream.
...
- **Urgent hang fix** — every Atlas call (bridge + client) is now timeout-bounded, so `hyscience project init` (run every session by the research prompt) and the per-command sync probe can't wedge a session for 60 min.
...
- Atlas managed compute: the `atlas compute:*` CLI suite (Modal sandboxes + reseller GPUs → `/api/compute/leases`) **exists in the published 0.13.2**, but HYscience pins `@hysci/atlas@^0.5.12` — a version-alignment gap, not a missing feature (see [06](06-compute-integrations.md)).
