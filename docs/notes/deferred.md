...
| **6**  | Compute / atlas version                  | ✅ `@hysci/atlas` `^0.5.12` → `^0.13.2` (managed compute resolves)                      |
| **7**  | Atlas experience — **A1 unified status** | ✅ `hyscience status` = connection + plan + wallet + usage + compute + companion        |
...
| —      | Atlas repo rebrand + sync-hash parity    | ✅ draft PR `HYscience/atlas#188`                                                       |
| —      | >60-min hang fix                         | ✅ all Atlas calls timeout-bounded; verified fail-fast e2e                              |

WS4/WS5/WS8 are now merged into `sprint/hyscience-atlas-polish` (PR #94). The WS8
...
One platform still shows three user-facing names ("Atlas" / "Thesis" / "hysci").
The wire identifiers (`hysci` provider id, `thk_`, `THESIS_*`) are contract and
**must stay**, but the internal names leak into copy — e.g. `status` prints
`Device: hysci · …` (the device name is stamped at login). This is a broad,
