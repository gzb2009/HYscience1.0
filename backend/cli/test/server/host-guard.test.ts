...
describe("isAllowedHost", () => {
  ...
  test("rejects the synthetic internal host (in-process calls use the nonce, not Host)", () => {
    expect(isAllowedHost("hyscience.internal")).toBe(false)
  })
...
describe("isAllowedOrigin", () => {
  ...
  test("allows the hosted web UI on a hyscience.ai subdomain", () => {
    expect(isAllowedOrigin("https://web.hyscience.ai")).toBe(true)
    expect(isAllowedOrigin("https://app.hyscience.ai")).toBe(true)
  })

  test("rejects the bare hyscience.ai apex (subdomain required)", () => {
    expect(isAllowedOrigin("https://hyscience.ai")).toBe(false)
  })
  ...
  test("rejects a lookalike domain", () => {
    expect(isAllowedOrigin("https://hyscience.ai.evil.com")).toBe(false)
    expect(isAllowedOrigin("https://evilhyscience.ai")).toBe(false)
  })

  test("rejects http (insecure) hyscience.ai", () => {
    expect(isAllowedOrigin("http://web.hyscience.ai")).toBe(false)
  })
