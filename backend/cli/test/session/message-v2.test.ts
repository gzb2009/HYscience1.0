...
describe("session.message-v2.toModelMessage", () => {
  ...
  test("isHybio accepts hybio and legacy synthetic", () => {
    expect(MessageV2.isHybio({ hybio: true })).toBe(true)
    expect(MessageV2.isHybio({ synthetic: true })).toBe(true)
    expect(MessageV2.isHybio({ hybio: false, synthetic: true })).toBe(true)
    expect(MessageV2.isHybio({})).toBe(false)
  })

  test("normalizePart migrates legacy synthetic to hybio", () => {
    const legacy = MessageV2.normalizePart({
      ...basePart("m-user", "p1"),
      type: "text",
      text: "hidden",
      synthetic: true,
    } as MessageV2.Part) as MessageV2.TextPart
    expect(legacy.hybio).toBe(true)
    expect(legacy.synthetic).toBeUndefined()
  })

  test("includes hybio text parts", () => {
    const messageID = "m-user"
