...
const mockPlugin = () => ({})
mock.module("hyscience-copilot-auth", () => ({ default: mockPlugin }))
mock.module("hyscience-anthropic-auth", () => ({ default: mockPlugin }))
mock.module("@gitlab/hyscience-gitlab-auth", () => ({ default: mockPlugin }))
...
test("provider request timeout defaults to five minutes unless disabled", () => {
  expect(Provider.requestTimeout(undefined)).toBe(300_000)
  expect(Provider.requestTimeout(60_000)).toBe(60_000)
  expect(Provider.requestTimeout(false)).toBe(false)
})
