...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const hyscienceModel = {
      ...openaiModel,
      providerID: "hysci",
      api: {
        id: "hyscience-test",
        url: "https://api.hyscience.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, hyscienceModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.hysci?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.hysci?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    expect(result[0].providerOptions?.hysci?.itemId).toBe("msg_hysci")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.hysci?.itemId).toBe("msg_hysci_part")
