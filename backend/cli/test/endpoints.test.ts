...
describe("endpoints.managedApiBase", () => {
  ...
  test("HYSC_API_BASE overrides the default", () => {
    expect(managedApiBase({ HYSC_API_BASE: "https://local.test" })).toBe("https://local.test")
  })
  ...
  test("HYSC_API_BASE wins over the other aliases", () => {
    expect(
      managedApiBase({
        HYSC_API_BASE: "https://a.test",
        MANAGED_API_BASE: "https://b.test",
        ATLAS_BASE_URL: "https://c.test",
      }),
    ).toBe("https://a.test")
    expect(managedApiBase({ HYSCIENCE_API_BASE: "https://n.test" })).toBe("https://n.test")
    expect(
      managedApiBase({
        HYSCIENCE_API_BASE: "https://n.test",
        HYSC_API_BASE: "https://a.test",
      }),
    ).toBe("https://n.test")
  })
  ...
  test("trailing slashes are stripped", () => {
    expect(managedApiBase({ HYSC_API_BASE: "https://x.test/" })).toBe("https://x.test")
    expect(managedApiBase({ HYSC_API_BASE: "https://x.test///" })).toBe("https://x.test")
  })
})
