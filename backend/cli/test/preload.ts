...
const dir = path.join(os.tmpdir(), "hyscience-test-data-" + process.pid)
...
process.env["HYSCIENCE_TEST_HOME"] = testHome

// Set test managed config directory to isolate tests from system managed settings
const testManagedConfigDir = path.join(dir, "managed")
process.env["HYSCIENCE_TEST_MANAGED_CONFIG_DIR"] = testManagedConfigDir
...
const cacheDir = path.join(dir, "cache", "hyscience")
...
// and never hit the network. With HYSCIENCE_DISABLE_MODELS_FETCH set, ModelsDev.Data
...
process.env["HYSCIENCE_DISABLE_MODELS_FETCH"] = "true"

// Disable builtin plugins that aren't published to npm (@hysci/anthropic-auth, etc.)
process.env["HYSCIENCE_DISABLE_DEFAULT_PLUGINS"] = "true"

// Disable bundled skills to isolate skill discovery tests
process.env["HYSCIENCE_DISABLE_BUNDLED_SKILLS"] = "true"
