...
describe.if(process.platform !== "win32")("File symlink boundary protection", () => {
  test("rejects read, write, and list through a symlink outside the project", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const external = path.join(dir, "..", `${path.basename(dir)}-external`)
        await fs.mkdir(external, { recursive: true })
        await Bun.write(path.join(external, "secret.txt"), "secret")
        await fs.symlink(external, path.join(dir, "escape"))
      },
    })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        await expect(File.write("escape/new.txt", "blocked")).rejects.toThrow(
          "Access denied: path escapes project directory",
        )
  ...
  test("allows a symlink whose resolved target remains inside the project", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await fs.mkdir(path.join(dir, "nested"))
        await Bun.write(path.join(dir, "nested", "allowed.txt"), "allowed")
        await fs.symlink(path.join(dir, "nested"), path.join(dir, "linked"))
      },
    })
    ...
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        expect((await File.read("linked/allowed.txt")).content).toBe("allowed")
        expect((await File.list("linked")).map((item) => item.name)).toEqual(["allowed.txt"])
      },
    })
  })
})
...
describe("Instance.containsPath", () => {
  ...
  test("returns true for path inside worktree but outside directory (monorepo subdirectory scenario)", async () => {
    ...
    await Instance.provide({
      directory: subdir,
      fn: () => {
        // .hyscience at worktree root, but we're running from packages/lib
        expect(Instance.containsPath(path.join(tmp.path, ".hyscience", "state"))).toBe(true)
