...
test("plan agent denies edits except .hyscience/plans/*", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(PermissionNext.evaluate("edit", ".hyscience/plans/foo.md", plan!.permission).action).toBe("allow")
    },
  })
})
...
test.skip("defaultAgent throws when all primary visible agents are disabled", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        research: { disable: true },
