...
describe("Project.fromDirectory", () => {
  test("git repository with no commits gets a stable path id", async () => {
    ...
    // The `.git/hyscience` cache file is no longer written — identity is path-derived.
    const hyscienceFile = path.join(tmp.path, ".git", "hyscience")
    expect(await Bun.file(hyscienceFile).exists()).toBe(false)
  })
...
describe("Project.fromDirectory migration", () => {
  test.skip("adopts a legacy per-directory record for the same folder", async () => {
    await using tmp = await tmpdir()
