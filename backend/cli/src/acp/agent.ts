...
import type { Event, HYscienceClient, SessionMessageResponse } from "@hysci/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: HYscienceClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `hyscience login` in the terminal",
        name: "Login with HYscience",
        id: "hyscience-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          if (part.text) {
            const audience: Role[] | undefined = MessageV2.isHybio(part)
              ? ["assistant"]
              : part.ignored
                ? ["user"]
                : undefined
          ...
          // HYscience stores files internally as { type: "file", url, filename, mime }.
    ...
    async prompt(params: PromptRequest) {
      ...
      const parts: Array<
        | { type: "text"; text: string; hybio?: boolean; ignored?: boolean }
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const hyscienceProvider = providers.find((p) => p.id === "hysci")
    if (hyscienceProvider) {
      if (hyscienceProvider.models["big-pickle"]) {
        return { providerID: "hysci", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(hyscienceProvider.models))
      if (best) {
        return {
