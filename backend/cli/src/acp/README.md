...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for hyscience.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal hyscience sessions
...
hyscience acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via hyscience's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | hyscience acp
```
...
### Mapping to HYscience

ACP sessions map cleanly to hyscience's internal session model:
