...
// hand-off to app.hyscience.ai/cli (the frontend opens the Plan tab) —
...
async function readWallet(): Promise<WalletState> {
  ...
  const [credits, mode, txns] = await Promise.all([
    HYscience.getCredits().catch(() => null),
    HYscience.getBillingMode().catch(() => null),
    HYscience.getTransactions(20).catch(() => null),
  ])
  return {
    signedIn: true,
