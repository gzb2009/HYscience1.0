...
// execute". Persists to a real JSON file under ~/.hyscience/ (Global.Path.data,
// mode 0600):
//
...
// every bash subprocess via HYscience.subprocessEnv. It runs at CLI/server
...
export namespace ComputeSettings {
  ...
  const EMPTY: Stored = { execution: "local", providers: {}, ssh_hosts: [], endpoints: [] }
  ...
  export async function applyComputeEnv(): Promise<void> {
    try {
      ...
      HYscience.registerSecretValues(secrets)
    } catch {
  ...
  function view(stored: Stored): Info {
    ...
    return {
      execution: stored.execution ?? "local",
      providers,
  ...
  export async function getExecution(): Promise<Execution> {
    const stored = await read()
    return stored.execution ?? "local"
  }

  export async function setExecution(execution: Execution): Promise<Info> {
    const stored = await read()
...
export const ComputeSettingsRoutes = lazy(() =>
  new Hono()
    ...
    .put(
      "/execution",
      describeRoute({
        summary: "Set compute execution tier",
        description: "Choose where agent runs execute: local kernels, SSH/Slurm hosts, or cloud GPUs.",
        operationId: "settings.compute.execution.set",
        responses: {
          200: { description: "Updated", content: { "application/json": { schema: resolver(ComputeSettings.Info) } } },
          ...errors(400),
        },
      }),
      validator("json", z.object({ execution: z.enum(["local", "ssh", "cloud"]) })),
      async (c) => c.json(await ComputeSettings.setExecution(c.req.valid("json").execution)),
    )
    .post(
      "/provider/:id",
      describeRoute({
