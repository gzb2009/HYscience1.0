...
async function readState(): Promise<BillingState> {
  ...
  const balanceUsd = (session ? await HYscience.getBalance().catch(() => null) : null) ?? -1
  return {
  ...
  new Hono()
    ...
    .put(
      ...
      async (c) => {
        ...
        // secrets into hyscience.json in plaintext.
        ...
        if (patch.llm) {
          await HYscience.setBillingMode(patch.llm).catch((e) =>
            log.warn("setBillingMode failed", { error: e instanceof Error ? e.message : String(e) }),
          )
          await HYscience.syncServices().catch((e) =>
