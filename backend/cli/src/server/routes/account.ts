...
export const AccountRoutes = lazy(() =>
  new Hono()
    .get(
      ...
      describeRoute({
        summary: "Get account",
        description: "Get synced HYscience account and billing summary.",
      ...
      async (c) => {
        ...
        const sync = session ? await HYscience.syncServices() : null
        // -1 is the wire encoding for "unknown" (schema: number)
        const balance = (session ? await HYscience.getBalance() : null) ?? -1
        const billing = session ? await HYscience.getBillingMode() : null
        return c.json({
          session: !!session,
          user: sync?.user,
          balance_usd: balance,
    ...
    .get(
      ...
      async (c) => c.json({ balance_usd: (await HYscience.getBalance()) ?? -1 }),
    )
    .get(
      "/devices",
      describeRoute({
      ...
      async (c) => c.json((await HYscience.listDevices()) ?? []),
    )
    .delete(
      "/devices/:keyID",
      describeRoute({
      ...
      async (c) => c.json(await HYscience.revokeDevice(c.req.valid("param").keyID)),
    )
    .get(
      "/billing-mode",
      describeRoute({
      ...
      async (c) => c.json(await HYscience.getBillingMode()),
    )
    .post(
      "/billing-mode",
      describeRoute({
      ...
      async (c) => c.json(await HYscience.setBillingMode(c.req.valid("json").mode)),
    )
    .post(
      "/login-key",
      describeRoute({
      ...
      async (c) => {
        ...
        try {
          await HYscience.loginWithKey(c.req.valid("json").key.trim())
    ...
    .post(
      ...
      async (c) => {
        ...
        await HYscience.revokeCurrentDevice()
