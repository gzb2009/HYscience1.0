// The local hyscience server only ever binds to loopback. Two independent checks
...
//     desktop (tauri) shell, *.hyscience.ai, and configured CORS
...
// The hosted web UI lives on <subdomain>.hyscience.ai; forks and
// self-hosters add their own via HYSCIENCE_CORS_DOMAINS (comma-separated apexes,
// e.g. "example.org,foo.dev"). Loopback + tauri stay auto-trusted regardless.
const DEFAULT_CORS_DOMAINS = ["hyscience.ai"]

function allowedApexDomains(): string[] {
  const configured = (process.env["HYSCIENCE_CORS_DOMAINS"] ?? "")
    .split(",")
...
export function isAllowedOrigin(origin: string, extraWhitelist: string[] = []): boolean {
  ...
  // Trusted hosted-UI subdomains (default hyscience.ai, extended via
  // HYSCIENCE_CORS_DOMAINS). See matchesAllowedSubdomain.
