...
<html>
  ...
  <body>
    <div class="container">
      ...
      <p>You can close this window and return to HYscience.</p>
    </div>
    <script>
...
export async function OpenaiSubscriptionAuthPlugin(input: PluginInput): Promise<Hooks> {
  return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            if (!currentAuth.access || currentAuth.expires < Date.now() + REFRESH_MARGIN_MS) {
              ...
              try {
                ...
                // hyscience processes (CLI + workspace server) race a
            ...
            try {
              ...
              throw new Error(
                `Couldn't start the browser sign-in listener on port ${OAUTH_PORT} ` +
                  ...
                  "`hyscience keys signin` and choose the device-code method.",
              )
            }
        ...
        },
        {
          ...
          authorize: async () => {
            const deviceResponse = await fetch(`${ISSUER}/api/accounts/deviceauth/usercode`, {
              ...
              headers: {
                "Content-Type": "application/json",
                "User-Agent": `hyscience/${Installation.VERSION}`,
              },
    ...
    "chat.headers": async (input, output) => {
      ...
      output.headers.originator = "hysci"
      output.headers["User-Agent"] =
        `hyscience/${Installation.VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`
