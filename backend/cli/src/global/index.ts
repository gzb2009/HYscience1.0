...
// Migration shim: installs created before the HYscience rename kept their
...
function migrateDir(base: string): string {
  ...
  if (existsSync(next) && existsSync(old)) {
    console.error(`hyscience: ignoring legacy config at ${old} (${next} already exists) — merge or remove it`)
  }
  return next
}
...
migrateFile(data, "hysci-session.json", "hyscience-session.json")
migrateFile(config, "synsc-synced.json", "hyscience-synced.json")
migrateFile(config, "synsc.jsonc", "hyscience.jsonc")
migrateFile(config, "synsc.json", "hyscience.json")

export namespace Global {
  export const Path = {
    // Allow override via HYSCIENCE_TEST_HOME for test isolation
    get home() {
      return process.env.HYSCIENCE_TEST_HOME || os.homedir()
    },
    data,
