...
// Backed by a plain JSON document per scope under ~/.hyscience data dir.
export namespace Memory {
  ...
  function fileFor(scope: Scope): string {
    ...
    const key = crypto.createHash("sha256").update(Instance.project.worktree).digest("hex").slice(0, 16)
  ...
  async function read(file: string) {
    ...
    try {
      ...
      log.error("failed to parse memory doc", { file, error: e })
    }
    return undefined
  }
  ...
  export async function get(scope: Scope): Promise<Doc> {
    ...
    if (scope === "global") return defaultDoc()

    const legacy = await read(legacyFile())
    if (!legacy) return defaultDoc()
    await fs.mkdir(path.dirname(file), { recursive: true })
    await Bun.write(file, JSON.stringify(legacy, null, 2))
    return legacy
  }
