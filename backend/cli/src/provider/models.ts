...
export namespace ModelsDev {
  ...
  function url() {
    return Flag.HYSCIENCE_MODELS_URL || "https://models.dev"
  }
  ...
  export const Data = lazy(async () => {
    ...
    if (result && (Flag.HYSCIENCE_DISABLE_MODELS_FETCH || hasCurrentFrontier(result))) return result
    // @ts-ignore
    const snapshot = await import("./models-snapshot")
      .then((m) => m.snapshot as Record<string, unknown>)
      .catch(() => undefined)
    if (snapshot && (Flag.HYSCIENCE_DISABLE_MODELS_FETCH || hasCurrentFrontier(snapshot as Record<string, any>)))
      return snapshot
    if (Flag.HYSCIENCE_DISABLE_MODELS_FETCH) return {}
...
if (!Flag.HYSCIENCE_DISABLE_MODELS_FETCH) {
  ModelsDev.refresh()
  setInterval(
    async () => {
