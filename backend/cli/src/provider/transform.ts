...
export namespace ProviderTransform {
  ...
  function fileToText(part: any): { type: "text"; text: string } {
    ...
    return {
      type: "text",
      text: `[Attached file ${name} (${mime}) is binary and can't be shown inline. If its contents are needed, ask the user to paste them or read it from .hyscience/context/.]`,
    }
  }
  ...
  export function options(input: {
    ...
    if (
      ...
      (input.model.providerID === "hysci" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
    ) {
    ...
    if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
      ...
      // "hysci" — but they route through the Atlas proxy baseURL. Reasoning
      ...
      if (input.model.providerID.startsWith("hysci") || viaManagedProxy) {
        result["promptCacheKey"] = input.sessionID
  ...
  export function error(providerID: string, error: APICallError) {
    ...
    if (providerID.includes("github-copilot") && error.statusCode === 403) {
      return "Please reauthenticate with the copilot provider to ensure your credentials work properly with HYscience."
    }
