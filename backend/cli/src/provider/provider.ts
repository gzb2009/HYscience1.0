...
export namespace Provider {
  ...
  export function requestTimeout(value: unknown) {
  ...
  async function managedProxyKey(baseURL: unknown): Promise<{ apiKey?: string }> {
    ...
    if (!managed) return {}
    const session = await HYscience.getSession().catch(() => null)
  ...
  function requireAtlasProxyForManagedKey(provider: Info, options: Record<string, any>) {
    ...
    throw new Error(
      `${provider.id} is using a managed Atlas key without an Atlas proxy URL. ` +
        "Run `hyscience connect sync` and try again.",
    )
  }

  /** A user-owned (BYOK) key: a real, non-managed credential. Excludes the
   *  "public" sentinel used for the zero-cost hyscience demo models. */
   ...
   * ANTHROPIC_BASE_URL → app.hyscience.ai). If the resolved key is the
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async hyscience(input) {
      const hasKey = await (async () => {
        const env = Env.all()
        if (input.env.some((item) => env[item])) return true
        if (await Auth.get(input.id)) return true
        const config = await Config.get()
        if (config.provider?.["hysci"]?.options?.apiKey) return true
        return false
      })()
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from hyscience.json provider config
    ...
    openrouter: async () => {
      ...
      return {
        ...
        options: {
          ...(baseURL ? { baseURL, ...(await managedProxyKey(baseURL)) } : {}),
          headers: {
            "HTTP-Referer": "https://hyscience.ai/",
            "X-Title": "hysci",
          },
        },
      }
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "hysci",
          },
        },
      }
    },
  ...
  function _hybioOpenRouterModel(modelID: string): Model {
    const m: Model = {
      id: modelID,
  ...
  async function _loadState() {
    ...
    // Legacy provider id from models.dev / older configs.
    if (database["synthetic"] && !database["hybio"]) {
      const legacy = database["synthetic"]
      database["hybio"] = {
        ...legacy,
        id: "hybio",
        name: legacy.name === "Synthetic" ? "Hybio" : legacy.name,
        env: Array.from(new Set([...(legacy.env ?? []), "HYBIO_API_KEY", "SYNTHETIC_API_KEY"])),
        models: mapValues(legacy.models, (model) => ({
          ...model,
          providerID: "hybio",
        })),
      }
    }
    delete database["synthetic"]

    const disabled = new Set((config.disabled_providers ?? []).map((id) => (id === "synthetic" ? "hybio" : id)))
    const enabled = config.enabled_providers
      ? new Set(config.enabled_providers.map((id) => (id === "synthetic" ? "hybio" : id)))
      : null
    ...
    for (const [rawID, provider] of configProviders) {
    ...
    // load apikeys
    for (const [rawID, provider] of Object.entries(await Auth.all())) {
      const providerID = resolveProviderID(rawID)
      if (disabled.has(providerID)) continue
    ...
    for (const [providerID, provider] of Object.entries(providers)) {
      ...
      for (const [modelID, model] of Object.entries(provider.models)) {
        ...
        if (model.status === "alpha" && !Flag.HYSCIENCE_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
      ...
      if (providerID === "openrouter" && configProvider?.whitelist) {
        for (const wlid of configProvider.whitelist) {
          if (!(wlid in provider.models)) {
            provider.models[wlid] = _hybioOpenRouterModel(wlid)
          }
        }
      }
  ...
  async function getSDK(model: Model) {
    try {
      ...
      options["fetch"] = async (input: any, init?: BunFetchRequestInit) => {
        ...
        const opts = init ?? {}
        const timeout = requestTimeout(options["timeout"])
  ...
  function resolveProviderID(providerID: string) {
    return providerID === "synthetic" ? "hybio" : providerID
  }

  export async function getProvider(providerID: string) {
    const id = resolveProviderID(providerID)
    return state().then((s) => s.providers[id])
  }
   ...
   *  `hyscience` provider participate in DEFAULT model selection — a fresh
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("hysci")) {
        priority = ["gpt-5-nano"]
      }
    ...
    // Only fall back to the hosted hyscience demo small-model when a managed
    ...
    if (await hasManagedSession()) {
      const hyscienceProvider = await state().then((state) => state.providers["hysci"])
      if (hyscienceProvider && hyscienceProvider.models["gpt-5-nano"]) {
        return getModel("hysci", "gpt-5-nano")
      }
    }
  ...
  export const NO_PROVIDER_HINT =
    "No model providers are available. Add your own API key (`hyscience keys add`) or connect a managed account (`hyscience login`), then choose a model."
  ...
  export async function defaultModel() {
    ...
    // Drop the hosted `hyscience` provider from DEFAULT priority unless a managed
    // session is active, then pick the first provider that actually has models.
    // Fall back to the raw configured list so a hyscience-only, unmanaged clone
    // still resolves a default rather than throwing.
    const candidates = providers.filter((p) => configured(p) && (managed || !p.id.startsWith("hysci")))
    const provider =
