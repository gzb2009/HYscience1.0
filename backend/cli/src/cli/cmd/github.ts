...
const AGENT_USERNAME = "hyscience-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/hyscience.yml"
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                step2,
                "",
                "    3. Go to a GitHub issue and comment `/hyscience summarize` to see the agent in action",
                "",
                "   Learn more about the GitHub agent - https://hyscience.ai/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/hyscience-agent"
            const command =
            ...
            async function getInstallation() {
              return await fetch(
                `https://app.hyscience.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
              )
                .then((res) => res.json())
...
jobs:
  hyscience:
    if: |
      ...
      contains(github.event.comment.body, ' /hyscience') ||
      startsWith(github.event.comment.body, '/hyscience')
    ...
    steps:
      ...
      - name: Run hyscience
        uses: HYscience/HYscience/github@latest${envStr}
        with:
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      try {
        ...
        console.log("hyscience session", session.id)
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://app.hyscience.ai"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/hyscience,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to hyscience...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("hyscience-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `hyscience/${type}-${hex}-${timestamp}`
        }
        return `hyscience/${type}${issueId}-${timestamp}`
      }
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the hyscience infrastructure after your response",
