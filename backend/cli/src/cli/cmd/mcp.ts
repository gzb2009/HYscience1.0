    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: hyscience mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in hyscience.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .hyscience/ subdirectory too)
  const candidates = [path.join(baseDir, "hyscience.json"), path.join(baseDir, "hyscience.jsonc")]

  if (!global) {
    candidates.push(
      path.join(baseDir, ".hyscience", "hyscience.json"),
      path.join(baseDir, ".hyscience", "hyscience.jsonc"),
    )
  }
  ...
  // Default to hyscience.json if none exist
  return candidates[0]
}
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "hyscience-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
