...
import { createHYscienceClient, type HYscienceClient } from "@hysci/sdk/v2"
...
export const RunCommand = cmd({
  ...
  builder: (yargs: Argv) => {
    return yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running hyscience server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    const execute = async (sdk: HYscienceClient, sessionID: string) => {
    ...
    if (args.attach) {
      const sdk = createHYscienceClient({ baseUrl: args.attach })
    ...
    await bootstrap(process.cwd(), async () => {
      const sdk = createHYscienceClient({ baseUrl: "http://hyscience.internal", fetch: Server.internalFetch() })
