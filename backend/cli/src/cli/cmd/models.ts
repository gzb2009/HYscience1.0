 ...
 *  - key starts with "thk_" → managed (the proxy thumbprint HYscience Cloud hands
 *    out on /api/cli/sync when the user has no BYOK key set).
 *  - options.baseURL points at HYscience Cloud (/api/llm/proxy/) → managed.
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        const providerIDs = Object.keys(providers).sort((a, b) => {
          const aIsManaged = a.startsWith("hysci")
          const bIsManaged = b.startsWith("hysci")
