...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run hyscience",
  builder: (yargs) =>
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
        ...
        UI.println()
        UI.println("Starting hyscience...")
        UI.println()

        const { spawn } = await import("child_process")
        const hyscienceProcess = spawn("hyscience", [], {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          hyscienceProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`hyscience exited with code ${code}`))
          })
          hyscienceProcess.on("error", reject)
        })
      },
    })
  },
