...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall hyscience and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall HYscience")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @hysci/hyscience",
      pnpm: "pnpm uninstall -g @hysci/hyscience",
      bun: "bun remove -g @hysci/hyscience",
      yarn: "yarn global remove @hysci/hyscience",
      brew: "brew uninstall hyscience",
      choco: "choco uninstall hyscience",
      scoop: "scoop uninstall hyscience",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall hyscience -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using HYscience!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# hyscience") || content.includes(".hyscience/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".hyscience/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".hyscience/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".hyscience"))
    ) {
      continue
    }
