...
async function openInEditor(initial: string): Promise<string> {
  ...
  const file = path.join(os.tmpdir(), `hyscience-skill-${Date.now()}.md`)
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (!info || !(info.location ?? "").includes("/user-skills/")) {
          UI.error(`No user skill named "${name}". Create one with: hyscience skill new ${name}`)
          process.exit(1)
          return
        }
...
const SkillListCommand = cmd({
  ...
  builder: (yargs: Argv) =>
    yargs.option("all", {
      describe: "include bundled HYscience skills in the listing",
      type: "boolean",
      default: false,
    }),
  handler: async (args) => {
    // Skill.state() needs project-instance context so it can walk the
    // .claude/.hyscience config dirs alongside the global cache + learned +
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        UI.println(`HYscience bundled skills: ${totalBundled}`)
        ...
        if (totalBundled === 0) {
          UI.println(
            "Bundled HYscience skills did not load. Run `hyscience login` and `hyscience sync`, then try again.",
          )
          UI.println("Offline fallback only shows skills already cached on this machine.")
          UI.println("")
        } else if (totalBundled < 100) {
          UI.println(
            "Only a small bundled skill set is visible; this usually means the full HYscience skill index was not fetched.",
          )
          UI.println("Run `hyscience connect sync` or try again once online/authenticated.")
          UI.println("")
        } else if (!showAll) {
          UI.println("Use `hyscience skill list --all` to show bundled HYscience skills.")
          UI.println("")
        }
        ...
        if (totalLearned === 0 && totalInstalled === 0) {
          UI.println("Install third-party skills with: hyscience skill add <git-url>")
          return
        }
        ...
        if (totalInstalled > 0) UI.println(`Run \`hyscience skill show <namespace>[/<name>]\` for details.`)
      },
    })
  },
})
  ...
  handler: async (args) => {
    ...
    if (target.includes("/")) {
      ...
      try {
        ...
        UI.println("")
        UI.println(`(SKILL.md not on disk at ${skillPath} — run \`hyscience\` to trigger sync)`)
      }
      return
    }
...
const SkillSetEntriesCommand = cmd({
  ...
  describe:
    ...
    "Writes hyscience-skills.json locally for the namespace.",
  builder: (yargs: Argv) => {
    return yargs
  ...
  handler: async (args) => {
    ...
    await fs.writeFile(path.join(nsDir, "hyscience-skills.json"), JSON.stringify({ entries }, null, 2))
    UI.println(
