...
import type { Hooks } from "@hysci/plugin"
    ...
    if (authorize.method === "auto") {
      ...
      try {
        ...
        if (result.type === "failed") {
          ...
          prompts.log.info("Declined, timed out, or cancelled. Retry with `hyscience keys signin`.")
...
export const AuthLoginCommand = cmd({
  ...
  builder: (yargs) =>
    yargs.positional("url", {
      describe: "hyscience auth provider",
      type: "string",
    }),
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
        ...
        if (args.url) {
          const wellknown = await fetch(`${args.url}/.well-known/hyscience`).then((x) => x.json() as any)
        ...
        if (provider === "other") {
          ...
          prompts.log.warn(
            `This only stores a credential for ${provider} - you will need to configure it in hyscience.json, check the docs for examples.`,
          )
        }
        ...
        if (provider === "amazon-bedrock") {
          prompts.log.info(
            "Amazon Bedrock authentication priority:\n" +
              ...
              "Configure via hyscience.json options (profile, region, endpoint) or\n" +
              "AWS environment variables (AWS_PROFILE, AWS_REGION, AWS_ACCESS_KEY_ID, AWS_WEB_IDENTITY_TOKEN_FILE).",
          )
        }

        if (provider === "hysci") {
          prompts.log.info("Create an API key at https://app.hyscience.ai/cli")
        }
        ...
        if (["cloudflare", "cloudflare-ai-gateway"].includes(provider)) {
          prompts.log.info(
            "Cloudflare AI Gateway can be configured with CLOUDFLARE_GATEWAY_ID, CLOUDFLARE_ACCOUNT_ID, and CLOUDFLARE_API_TOKEN environment variables. Read more: https://hyscience.ai/docs/providers/#cloudflare-ai-gateway",
          )
        }
