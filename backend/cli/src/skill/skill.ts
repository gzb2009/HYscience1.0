...
export namespace Skill {
  ...
  export const Info = z.object({
     ...
     *  Driven by `hyscience-skills.json` `entries[]` for URL-installed skills;
  ...
  const HYSCIENCE_SKILL_GLOB = new Bun.Glob("{skill,skills}/**/SKILL.md")
  ...
  async function compute() {
    ...
    if (!Flag.HYSCIENCE_DISABLE_CLAUDE_CODE_SKILLS) {
    ...
    // Scan .hyscience/skill/ directories
    for (const dir of await Config.directories()) {
      for await (const match of HYSCIENCE_SKILL_GLOB.scan({
        cwd: dir,
        absolute: true,
        onlyFiles: true,
        followSymlinks: true,
    ...
    // === Bundled skills (local tree shipped with the CLI) ===
    if (!Flag.HYSCIENCE_DISABLE_BUNDLED_SKILLS) {
      ...
      if (await Filesystem.isDir(bundledDir)) {
        ...
        log.info("Loaded bundled skills", { path: bundledDir, count })
      }

      const cachedSkillsDir = path.join(Global.Path.cache, "skills")
      if (await Filesystem.isDir(cachedSkillsDir)) {
        ...
        if (count > 0) log.info("Loaded cached skills", { count })
      }
    }

    // === Learned Skills: local RSI distillation ===
    ...
    // === User Skills: authored locally via hyscience/web, private by default ===
    ...
    //   ~/.hyscience/installed-skills/<ns>/skills/<name>/SKILL.md
    ...
    if (await Filesystem.isDir(installedDir)) {
      ...
      try {
        ...
        for (const ns of nsDirs) {
          if (!ns.isDirectory()) continue
          const manifestPath = path.join(installedDir, ns.name, "hyscience-skills.json")
          try {
