...
async function makeFixture(): Promise<string> {
  const dir = await mkdtemp(path.join(os.tmpdir(), "hyscience-fixture-"))
...
describe("fetchManifest", () => {
  ...
  it("reads hyscience-skills.json entries from repo root when present", async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), "hyscience-manifest-"))
    ...
    await writeFile(path.join(dir, "hyscience-skills.json"), JSON.stringify({ entries: ["public-skill"] }))
    await $`git init -q`.cwd(dir).quiet()
    await $`git add -A`.cwd(dir).quiet()
    await $`git -c user.email=t@t -c user.name=t commit -q -m init`.cwd(dir).quiet()
  ...
  it("rejects when no skills/**/SKILL.md found", async () => {
    const empty = await mkdtemp(path.join(os.tmpdir(), "hyscience-empty-"))
