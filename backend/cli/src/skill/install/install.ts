...
function installedDir(): string {
  ...
  // Allow tests to override via HYSCIENCE_DATA_DIR without monkey-patching globals.
  const base = process.env.HYSCIENCE_DATA_DIR ?? Global.Path.data
...
export namespace Install {
  ...
  export async function add(url: string, options: InstallOptions = {}): Promise<InstallResult> {
    ...
    try {
      ...
      if (!skipClassifier && surviving.length > 0) {
        const review = await HYscience.requestSkillReview(
          surviving.map((s) => ({
      ...
      try {
        ...
        for (const skill of surviving) {
          ...
          if (!skipClassifier) {
            ...
            await HYscience.postInstalledSkill({
  ...
  export async function remove(target: string): Promise<{ archived: number }> {
    ...
    if (name) {
      ...
      const ok = await HYscience.deleteInstalledSkill(namespace, name).catch(() => false)
    ...
    await fs.rm(dir, { recursive: true, force: true }).catch(() => {})
    const result = await HYscience.deleteInstalledNamespace(namespace).catch(() => null)
  ...
  export async function list(): Promise<{ namespace: string; name: string; description: string; verdict: string }[]> {
    const rows = await HYscience.fetchInstalledSkills()
    return (rows ?? []).map((r) => ({
