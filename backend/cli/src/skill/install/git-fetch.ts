...
export async function gitFetchPinned(params: FetchPinnedParams): Promise<void> {
  const tmpDir = await mkdtemp(path.join(os.tmpdir(), "hyscience-skill-sync-"))
  try {
    ...
    // Also copy the namespace's hyscience-skills.json (entries manifest) if
    ...
    const nsManifest = path.join(nsDir, "hyscience-skills.json")
    const upstreamManifest = path.join(tmpDir, "hyscience-skills.json")
