...
export interface FetchResult {
  ...
  tmpDir: string
  manifest: SkillEntry[]
  /** Repo's declared user-facing entry points (from `hyscience-skills.json` at
...
export async function fetchManifest(parsed: ParsedSkillUrl): Promise<FetchResult> {
  const tmpDir = await mkdtemp(path.join(os.tmpdir(), "hyscience-skill-"))
  ...
  // Optional manifest: `hyscience-skills.json` at repo root listing entries.
  // Tolerate malformed JSON (treat as absent rather than failing the install).
  let entries: string[] | null = null
  try {
    const manifestPath = path.join(tmpDir, "hyscience-skills.json")
