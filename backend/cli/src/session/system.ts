...
export namespace SystemPrompt {
  const CREDENTIAL_SERVICES: Array<[string[], string]> = [
  ...
  export function credentialStatus() {
    const connected = CREDENTIAL_SERVICES.filter(([keys]) => keys.every((key) => process.env[key])).map(
      ([, label]) => label,
    )
    if (connected.length === 0) {
      return "<credentials>No external service credentials detected in this process.</credentials>"
    }
