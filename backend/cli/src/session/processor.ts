...
export namespace SessionProcessor {
  ...
  export function isDoomLoop(parts: MessageV2.Part[], tool: string, input: Record<string, unknown>) {
    const recent = parts.filter((part): part is MessageV2.ToolPart => part.type === "tool").slice(-DOOM_LOOP_THRESHOLD)
    return (
              ...
              for await (const value of stream.fullStream) {
                ...
                switch (value.type) {
                  ...
                  case "reasoning-delta":
                    if (value.id in reasoningMap) {
                      guard.reasoningDelta(value.id, value.text)
                      guard.check()
