...
export namespace MessageV2 {
  ...
  export const TextPart = PartBase.extend({
    type: z.literal("text"),
    text: z.string(),
    /** Agent-only / system-injected text; hidden from the user-facing transcript. */
    hybio: z.boolean().optional(),
    /** @deprecated Prefer `hybio`. Accepted for older clients and stored sessions. */
  ...
  /** True when a text part is agent-only (new `hybio` or legacy `synthetic`). */
  export function isHybio(part: { hybio?: boolean; synthetic?: boolean }) {
    return !!(part.hybio || part.synthetic)
  }

  /** Normalize legacy `synthetic` onto `hybio` when reading stored parts. */
  export function normalizePart<T extends Part>(part: T): T {
    if (part.type !== "text") return part
    if (part.hybio !== undefined || part.synthetic === undefined) {
      if (part.synthetic === undefined) return part
      const { synthetic: _legacy, ...rest } = part
      return rest as T
    }
    const { synthetic: legacy, ...rest } = part
    return { ...rest, hybio: legacy } as T
  }
  ...
  export const parts = fn(Identifier.schema("message"), async (messageID) => {
    ...
    for (const item of await Storage.list(["part", messageID])) {
      ...
      result.push(normalizePart(read))
    }
