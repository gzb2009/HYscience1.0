...
export namespace RSITrajectory {
  ...
  export async function capture(sessionID: string): Promise<Trajectory | null> {
    try {
      ...
      if (!hypothesis) {
        ...
        if (firstUser) {
          const textPart = firstUser.parts.find((p: any) => p.type === "text" && !MessageV2.isHybio(p))
  ...
  export async function pipeline(sessionID: string): Promise<void> {
    try {
      ...
      // Scan session messages for hypothesis blocks and test results
      Session.messages({ sessionID })
        .then((msgs) => Hypothesis.scanSession(sessionID, msgs))
        .catch(() => {})
      if (!trajectory) return
