...
export const SkillTool = Tool.define("skill", async (ctx) => {
  ...
  const description =
    accessibleSkills.length === 0
      ...
      : [
          ...
          "Use `query` to search skills by name, description, or tags; use `name` to load a specific skill directly; use `category` to browse available skills in a domain.",
          "",
  ...
  const parameters = z.object({
    ...
    query: z
      .string()
      .optional()
      .describe("Search skills dynamically by name, description, or tags (e.g., 'single-cell', 'GPU training')"),
  })
  ...
  return {
    ...
    async execute(
      ...
      // Dynamic search mode: newly added skills appear here automatically.
      if (params.query) {
        const q = params.query.toLowerCase()
        const scored = accessibleSkills
          .map((s) => {
            const nameScore = fuzzyScore(q, s.name)
            const descriptionScore = fuzzyScore(q, s.description)
            const tagScore = Math.max(0, ...(s.tags ?? []).map((t) => fuzzyScore(q, t)))
            const score = Math.max(nameScore, descriptionScore * 0.8, tagScore)
        ...
        if (scored.length === 0) {
          return {
            ...
            output: `No skills matched "${params.query}". Try a broader query or use skill(category="<category>") to browse.`,
        ...
        const listing = scored
          ...
          .join("\n")
        return {
          title: `Skills matching: ${params.query} (${scored.length})`,
          output: `## Skills matching "${params.query}"\n\n${listing}\n\nLoad one by calling skill(name="<name>").`,
      ...
      if (isCachedSkill) {
        ...
        if (!hasContent) throw new Error(`Skill "${name}" not available locally (not cached)`)
      }
