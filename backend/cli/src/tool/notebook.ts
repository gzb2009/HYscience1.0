...
_real_out.write("__HYSCIENCE_KERNEL_READY__\\n")
_real_out.flush()
...
while True:
    ...
    _real_out.write("__HYSCIENCE_RESULT_START__\\n" + r + "\\n__HYSCIENCE_RESULT_END__\\n")
    _real_out.flush()
`.trim()

const READY = "__HYSCIENCE_KERNEL_READY__"
const START = "__HYSCIENCE_RESULT_START__\n"
const END = "\n__HYSCIENCE_RESULT_END__"
...
class PythonKernel implements Kernel {
  ...
  async start(opts?: KernelStartOptions): Promise<void> {
    ...
    const scriptPath = path.join(os.tmpdir(), `hyscience-pykernel-${this.id.slice(0, 8)}-${Date.now()}.py`)
