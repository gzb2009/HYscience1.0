...
run_cell <- function(code) {
  ...
  cat("__HYSCIENCE_R_RESULT_START__\\n")
  cat("OK:", if (ok) "1" else "0", "\\n", sep = "")
  cat("IMG:", imgpath, "\\n", sep = "")
  cat("__HYSCIENCE_R_OUT__\\n")
  cat(out_text)
  cat("\\n__HYSCIENCE_R_MSG__\\n")
  cat(msg_text)
  cat("\\n__HYSCIENCE_R_END__\\n")
...
cat("__HYSCIENCE_KERNEL_READY__\\n")
flush(stdout())
...
repeat {
  ...
  repeat {
    ...
    if (identical(l, "__HYSCIENCE_CODE_END__")) { got_end <- TRUE; break }
    lines <- c(lines, l)
  }
...
const READY = "__HYSCIENCE_KERNEL_READY__"
const START = "__HYSCIENCE_R_RESULT_START__\n"
const END = "\n__HYSCIENCE_R_END__"
...
function parseFrame(block: string): RawResult {
  const outMarker = "__HYSCIENCE_R_OUT__\n"
  const msgMarker = "\n__HYSCIENCE_R_MSG__\n"
...
class RKernel implements Kernel {
  ...
  async start(opts?: KernelStartOptions): Promise<void> {
    ...
    const scriptPath = path.join(os.tmpdir(), `hyscience-rkernel-${this.id.slice(0, 8)}-${Date.now()}.R`)
