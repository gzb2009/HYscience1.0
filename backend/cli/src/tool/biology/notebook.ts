...
_out.write("__HYSCIENCE_KERNEL_READY__\\n")
_out.flush()
...
while True:
    ...
    _out.write("__HYSCIENCE_RESULT_START__\\n" + r + "\\n__HYSCIENCE_RESULT_END__\\n")
    _out.flush()
`.trim()
...
async function getKernel(sessionID: string): Promise<Kernel> {
  ...
  // Start new kernel
  const scriptPath = path.join(os.tmpdir(), `hyscience-kernel-${sessionID.slice(0, 8)}-${Date.now()}.py`)
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const handler = (data: Buffer) => {
      buf += data.toString()
      if (buf.includes("__HYSCIENCE_KERNEL_READY__")) {
...
function executeInKernel(
  ...
  return new Promise((resolve, reject) => {
    ...
    const handler = (data: Buffer) => {
      ...
      const startMarker = "__HYSCIENCE_RESULT_START__\n"
      const endMarker = "\n__HYSCIENCE_RESULT_END__"
