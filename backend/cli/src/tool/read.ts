...
export const ReadTool = Tool.define("read", {
  ...
  async execute(params, ctx) {
    ...
    let truncatedByLine = false
    for (let i = offset; i < Math.min(lines.length, offset + limit); i++) {
      const clipped = lines[i].length > MAX_LINE_LENGTH
      const line = clipped ? lines[i].substring(0, MAX_LINE_LENGTH) + "..." : lines[i]
      if (clipped) truncatedByLine = true
    ...
    const truncated = hasMoreLines || truncatedByBytes || truncatedByLine

    if (truncatedByBytes) {
      output += `\n\n(Output truncated at ${MAX_BYTES} bytes. Use 'offset' parameter to read beyond line ${lastReadLine})`
    } else if (hasMoreLines) {
      output += `\n\n(File has more lines. Use 'offset' parameter to read beyond line ${lastReadLine})`
    } else if (truncatedByLine) {
      output += `\n\n(Line content truncated at ${MAX_LINE_LENGTH} characters)`
    } else {
