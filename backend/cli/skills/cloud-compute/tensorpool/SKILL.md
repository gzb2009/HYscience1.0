...
Credentials are auto-injected by hyscience when connected via the dashboard.
...
If not set: connect TensorPool at https://app.hyscience.ai -> Services, then restart hyscience.
...
```bash
# Set API key (synced automatically via HYscience dashboard)
...
If connected via the HYscience dashboard, `TENSORPOOL_KEY` is injected automatically.
...
```bash
[ -n "$TENSORPOOL_KEY" ] && echo "set" || echo "not set"
# If not set, connect via HYscience dashboard or export manually
```
...
When the `hyscience` agent loads this skill for a user task:
