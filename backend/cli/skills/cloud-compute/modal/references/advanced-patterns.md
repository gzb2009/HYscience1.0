...
Advanced patterns for Modal including disconnect-safe training, multi-node training, distributed primitives, sandbox workflows, memory snapshots, and integration with hyscience/other skills.
...
## hyscience Integration

### Credential Handling

Modal credentials are injected via HYscience. The Modal CLI reads `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` from environment automatically.
