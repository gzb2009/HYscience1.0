...
## Credential Setup (hyscience)

Credentials are auto-injected via HYscience. Verify before running Modal workloads:
...
| Advanced Patterns & hyscience Integration | [Advanced Patterns](references/advanced-patterns.md) |
...
4. Adapt for your use case using hyscience credentials (`MODAL_TOKEN_ID`/`MODAL_TOKEN_SECRET`)
5. After job completes, report usage via `HYscience.reportUsage()` with `service="modal"`
