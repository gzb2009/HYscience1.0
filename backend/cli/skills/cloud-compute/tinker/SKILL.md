...
# TINKER_API_KEY must be set — connect Tinker in the HYscience dashboard to sync your API key.
...
print(f'\n[HYSCIENCE_USAGE] {json.dumps({"service": "tinker", "event_type": "training", "model": model_name, "tokens_used": total_tokens})}')
```
...
**CRITICAL**: All training scripts MUST print a `[HYSCIENCE_USAGE]` line at the end. The CLI automatically captures this and reports exact billing to the dashboard.
...
The CLI bash tool scans output for `[HYSCIENCE_USAGE]` markers and auto-reports to the dashboard — no manual reporting needed.

## Common Issues

| Problem | Solution |
|---------|----------|
| `TINKER_API_KEY` not set | `export TINKER_API_KEY=your_key` or check HYscience credential sync |
