...
These skills provide agent-friendly workflows that the `hyscience` CLI can invoke.
...
If connected via the HYscience dashboard, `PRIME_API_KEY` is injected automatically.
...
9. **Report usage**: After completion, report via `HYscience.reportUsage()` with `service="primeintellect"`
