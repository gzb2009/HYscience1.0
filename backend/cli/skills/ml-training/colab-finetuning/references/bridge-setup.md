...
```
hyscience CLI ←→ WebSocket ←→ Cloudflare Tunnel ←→ Jupyter Server (on Colab GPU)
```
...
4. hyscience connects via WebSocket and sends Jupyter kernel messages
...
```
This creates `hyscience-bridge.ipynb` in your project directory.

### 2. Upload to Google Colab
- Go to [colab.research.google.com](https://colab.research.google.com)
- File → Upload notebook → select `hyscience-bridge.ipynb`
