...
description: Fine-tune LLMs on Google Colab GPUs directly from hyscience. Connects to Colab runtimes via WebSocket bridge for remote training with Unsloth. Supports SFT, GRPO, DPO, vision, and TTS workflows on free T4 to Pro A100 GPUs.
...
Fine-tune LLMs using Google Colab GPUs directly from the hyscience CLI. Connect to free or paid Colab runtimes and run Unsloth training workflows remotely.
...
This creates a `hyscience-bridge.ipynb` file that establishes a WebSocket tunnel between hyscience and the Colab GPU.
