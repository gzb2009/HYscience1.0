...
**This is expected in hyscience.** The backend is set to `Agg` (headless) for all subprocesses. `plt.show()` is a no-op — it does not pop up a window and it does not save anything.

**Solution: Save to file instead.**
```python
# Always save figures to files — do not use plt.show()
import matplotlib
matplotlib.use('Agg')  # already set by hyscience subprocess env, but safe to repeat
import matplotlib.pyplot as plt

fig, ax = plt.subplots()
ax.plot([1, 2, 3])
fig.savefig("my_plot.png", dpi=150, bbox_inches="tight")
# Then `read my_plot.png` to verify
```

### Issue: "RuntimeError: main thread is not in main loop"

**Problem:** Interactive backend conflicts with non-main-thread execution.

**This is already handled by hyscience** — `MPLBACKEND=Agg` is set in all subprocesses. If you still see this error, explicitly set the backend before any other matplotlib import:
```python
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
...
**Problem:** Interactive mode is not available in hyscience (headless `Agg` backend).

**This is the intended behavior.** Re-render the figure with updated data and save to a new file.
```python
fig, ax = plt.subplots()
ax.plot(data)
fig.savefig("updated_plot.png", dpi=150)
```
