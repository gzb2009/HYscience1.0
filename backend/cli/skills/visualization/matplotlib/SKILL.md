...
> **CRITICAL:** Matplotlib runs headless in hyscience (MPLBACKEND=Agg). GUI windows cannot appear. **Never call `plt.show()`** — it does nothing and may hang the process. Always save figures to files with `fig.savefig("filename.png", dpi=150, bbox_inches="tight")` or `plt.savefig(...)`. After saving, `read` the file to verify it rendered correctly.
