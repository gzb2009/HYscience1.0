...
def _synced_env() -> dict:
    """Read HYscience's managed synced env (written by `hyscience connect sync`).
    ...
    try:
        with open(os.path.join(base, "hyscience", "synced-env.json"), encoding="utf-8") as f:
            data = json.load(f)
...
"""
    ...
    def __init__(self, api_key: Optional[str] = None, verbose: bool = False):
        ...
        """
        # Priority: 1) explicit param, 2) env var, 3) .env file, 4) HYscience managed credential
        synced = _synced_env()
        ...
        # Fall back to the HYscience managed credential (routes through the Atlas
        ...
        if not self.api_key:
            raise ValueError(
                "OPENROUTER_API_KEY not found. Connect OpenRouter via `hyscience login` or at\n"
                "https://app.hyscience.ai → Services, or set OPENROUTER_API_KEY for this run.\n"
