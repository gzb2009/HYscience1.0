...
Unless the user specifies a journal or house style, use the supplied Nature-style preset as the
default starting point for final research figures:
...
This means a clean white canvas, readable sans-serif typography, restrained color, direct labels,
minimal gridlines/spines, and a layout that can be understood at manuscript column width. It does
not replace checking the final target journal's technical requirements.

Never submit a library-default plot as a final artifact. Before export, verify:

- Data marks remain legible at final size; do not make large point clouds so faint that structure disappears.
- Labels, legends, and annotations are readable without zooming and do not overlap data.
- Colors encode meaning, are colorblind-safe, and remain distinguishable in grayscale where feasible.
- Every panel has a purpose; multi-panel figures share typography, scales where comparison requires it, and panel labels.
- Save both a review PNG and a publication PDF; rasterize only dense point layers in the PDF when necessary.
...
### 6. Embeddings, Spatial Maps, and Dense Point Clouds

For UMAP, t-SNE, spatial coordinates, and other dense scatter plots, optimize the final figure
for interpretable structure rather than plotting every default marker unchanged:

- Use the same embedding limits, point size, and alpha across panels that are meant to compare the
  same cells. Make point size responsive to the number of observations.
- Use a categorical palette with enough contrast for cell populations; retain the same color for a
  cell type across all figures in a project.
- Prefer direct cluster/type labels with a light outline or a well-spaced legend. Do not leave a
  tiny, dense, unreadable legend at the edge of a large canvas.
- For two-panel annotation figures, clearly distinguish the analytical view (for example, clusters)
  from the biological interpretation (for example, cell types) with concise panel titles and
  aligned axes.
- Rasterize only the scatter collection for very large datasets, while keeping text, axes, and
  annotations vector-sharp in PDF output.
- Inspect the exported PNG at its intended display size. If labels, points, or legends are too
  small to read, revise the figure before reporting it.
