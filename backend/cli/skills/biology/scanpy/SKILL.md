...
dependencies: ["scanpy>=1.10.0", "anndata>=0.10.0", "numpy>=1.25.0", "openpyxl>=3.1.0"]
---
...
Deterministic contract — QC → normalize → HVG → PCA → neighbors → leiden → markers:

```bash
python scripts/pipeline.py --input path/to/data.h5ad --output-dir ./scrna_out
```

Writes `scrna_out/adata.h5ad`, `markers.csv`, figures, and `_script_manifest.jsonl`.
After success, record provenance on key outputs (`provenance_record` with `code_path=scripts/pipeline.py`).

### Annotation report (subset naming)

After assigning every cluster a name, write JSON then:

```bash
python scripts/annotation_report.py \
  --input annotations.json \
  --markers markers.csv \
  --output MacDC_annotation.xlsx \
  --locale zh   # or en — match UI locale
```

When a markers CSV/TSV exists, `--markers` is required and verifies that every source cluster is represented exactly once.
Sheets: subset summary · keep-for-downstream · contamination · key-findings (locale-aware headers).
