...
for (const filepath of new Bun.Glob("@hysci/*/package.json").scanSync({ cwd: "./dist" })) {
  ...
  JSON.stringify(
    {
      name: pkg.name,
      bin: {
        hyscience: `./bin/hyscience`,
      },
      scripts: {
        // best-effort: clears a stale global @hysci/cli whose `hyscience`
      ...
      repository: {
        type: "git",
        url: "git+https://github.com/HYscience/hyscience.git",
      },
...
for (const [name] of Object.entries(binaries)) {
  try {
    if (!name.includes("windows")) {
      await $`chmod 755 ./dist/${name}/bin/hyscience`
    }
...
if (failed.length > 0) {
  ...
  throw new Error("Refusing to publish @hysci/hyscience wrapper because one or more platform packages failed")
}
...
if (!Script.preview) {
  try {
    ...
    const arm64Sha = await $`sha256sum ./dist/hyscience-linux-arm64.tar.gz | cut -d' ' -f1`.text().then((x) => x.trim())
    const x64Sha = await $`sha256sum ./dist/hyscience-linux-x64.tar.gz | cut -d' ' -f1`.text().then((x) => x.trim())
    const macX64Sha = await $`sha256sum ./dist/hyscience-darwin-x64.zip | cut -d' ' -f1`.text().then((x) => x.trim())
    const macArm64Sha = await $`sha256sum ./dist/hyscience-darwin-arm64.zip | cut -d' ' -f1`
      .text()
      .then((x) => x.trim())
    ...
    const homebrewFormula = [
      ...
      `  homepage "https://github.com/HYscience/HYscience"`,
      ...
      `      url "https://github.com/HYscience/HYscience/releases/download/v${Script.version}/hyscience-darwin-x64.zip"`,
      `      sha256 "${macX64Sha}"`,
      "",
      "      def install",
      '        bin.install "hyscience"',
      "      end",
      "    end",
      "    if Hardware::CPU.arm?",
      `      url "https://github.com/HYscience/HYscience/releases/download/v${Script.version}/hyscience-darwin-arm64.zip"`,
      `      sha256 "${macArm64Sha}"`,
      "",
      "      def install",
      '        bin.install "hyscience"',
      ...
      `      url "https://github.com/HYscience/HYscience/releases/download/v${Script.version}/hyscience-linux-x64.tar.gz"`,
      `      sha256 "${x64Sha}"`,
      "      def install",
      '        bin.install "hyscience"',
      "      end",
      ...
      `      url "https://github.com/HYscience/HYscience/releases/download/v${Script.version}/hyscience-linux-arm64.tar.gz"`,
      `      sha256 "${arm64Sha}"`,
      "      def install",
      '        bin.install "hyscience"',
      "      end",
    ...
    if (!token) {
      ...
      const tap = `https://x-access-token:${token}@github.com/HYscience/homebrew-tap.git`
      await $`rm -rf ./dist/homebrew-tap`
      await $`git clone ${tap} ./dist/homebrew-tap`
      await Bun.file("./dist/homebrew-tap/hyscience.rb").write(homebrewFormula)
      await $`cd ./dist/homebrew-tap && git add hyscience.rb`
