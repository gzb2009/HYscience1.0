...
// Build the hyscience web UI (frontend/workspace) and regenerate the embedded asset
...
console.log("building frontend/workspace (hyscience web)")
...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      outfile: `dist/${name}/bin/hyscience`,
      execArgv: [`--user-agent=hyscience/${Script.version}`, "--use-system-ca", "--"],
      windows: {},
    },
    entrypoints: ["./src/index.ts"],
    define: {
      ...keyDefines,
      HYSCIENCE_VERSION: `'${Script.version}'`,
      HYSCIENCE_CHANNEL: `'${Script.channel}'`,
      HYSCIENCE_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "",
    },
  })
...
if (Script.release) {
  ...
  for (const key of Object.keys(binaries)) {
    const archiveName = key.replace(`${pkg.name}-`, "hyscience-") + (key.includes("linux") ? ".tar.gz" : ".zip")
