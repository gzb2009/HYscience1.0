...
All notable changes to HYscience are recorded here. The project follows
[semantic versioning](https://semver.org). Releases are cut from `main` via the
`publish` workflow and published to npm as
[`@hysci/hyscience`](https://www.npmjs.com/package/@hysci/hyscience); each
...
- Unified `hyscience status`: connection, plan, wallet balance + lifetime
...
- Bundled `@hysci/atlas` companion bumped to `^0.13.2` so managed compute
  resolves.
...
- HYscience docs site at hyscience.sh/docs.
