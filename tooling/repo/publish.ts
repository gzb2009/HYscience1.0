...
import { $ } from "bun"
import { Script } from "@hysci/script"
...
try {
  ...
  // Pin @hysci/hyscience dependency to the version being published.
  ...
  // Keep the launcher version in lockstep with the just-released @hysci/hyscience.
  ...
  // Do NOT declare @hysci/hyscience as a static dependency. Both packages
  // expose a `hyscience` bin and npx resolves dep-bin before parent-bin,
  // which caused `npx hyscience` to skip the launcher and jump straight
  // into the CLI (npm caches: hyscience -> @hysci/hyscience/bin/hyscience instead
  // of hyscience -> hyscience/bin/hyscience.mjs). The launcher already shells
  // out `npm i -g @hysci/hyscience@latest` at runtime when it needs the
  // binary — that path puts hyscience on PATH for the subsequent spawn
  ...
  if (result.exitCode !== 0) {
    ...
    // The unscoped `hysci` package has its own npm owner list; a token
    // whose account isn't on it gets E403. That's an ownership grant to
    // chase (`npm owner add <token-user> hysci`), not a broken release —
    ...
    if (stderr.includes("E403") || stderr.includes("do not have permission")) {
      ...
      console.warn(
        "::warning title=launcher not published::npm token's account is not an owner of the 'hysci' package — users keep getting the previous launcher. Fix: an owner runs `npm owner add <token-user> hysci`, then re-release.",
      )
      ...
      console.warn(
        `::warning title=launcher skipped::hysci@${Script.version} already exists on the registry (published out-of-band) — the release-built launcher was NOT published.`,
      )
    } else {
