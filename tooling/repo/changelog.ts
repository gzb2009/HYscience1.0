...
import { $ } from "bun"
import { createHYscience } from "@hysci/sdk/v2"
import { parseArgs } from "util"

export const team = ["ishaan1124", "hyscience", "hyscience-agent[bot]", "actions-user"]
...
export async function getLatestRelease(skip?: string) {
  ...
  const data = await fetch("https://api.github.com/repos/HYscience/HYscience/releases?per_page=100", {
    headers,
  }).then((res) => {
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/HYscience/HYscience/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
...
async function summarizeCommit(
  hyscience: Awaited<ReturnType<typeof createHYscience>>,
  message: string,
): Promise<string> {
  console.log("summarizing commit:", message)
  const session = await hyscience.client.session.create()
  const result = await hyscience.client.session
    .prompt(
      {
        sessionID: session.data!.id,
        model: { providerID: "hysci", modelID: "claude-sonnet-4-5" },
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], hyscience: Awaited<ReturnType<typeof createHYscience>>) {
  // Summarize commits in parallel with max 10 concurrent requests
  const BATCH_SIZE = 10
  const summaries: string[] = []
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    const batch = commits.slice(i, i + BATCH_SIZE)
    const results = await Promise.all(batch.map((c) => summarizeCommit(hyscience, c.message)))
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/HYscience/HYscience/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
export async function buildNotes(from: string, to: string) {
  ...
  const hyscience = await createHYscience({ port: 0 })
  const notes: string[] = []

  try {
    const lines = await generateChangelog(commits, hyscience)
    notes.push(...lines)
